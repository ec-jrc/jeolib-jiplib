...
//             std::cout << std::endl;
      ...
      std::vector<fann_type> run(std::vector<fann_type> input)
        ...
        */
        ...
        bool create_from_file(const std::string &configuration_file, std::vector<double> &offset, std::vector<double> &scale, std::map<std::string,short> &classValueMap, std::vector<std::string> &nameVector, std::vector<std::string> &bandNames){
          offset.clear();
          scale.clear();
          classValueMap.clear();
          nameVector.clear();
          bandNames.clear();
          create_from_file(configuration_file.c_str());
          short verbose=0;
          std::ifstream m_ifstream;
          m_ifstream.open(configuration_file.c_str(),std::ios_base::in);
          if(!(m_ifstream)){
            std::ostringstream errorStream;
            errorStream << "Error: could not open file" << configuration_file;
          ...
          char m_comment='#';
          char m_equal='=';
          char m_fs=',';//commma seperated
          bool isComment=false;
          bool isEqual=false;
          std::string csvRecord;
          std::string item;
          size_t pos=std::string::npos;
          size_t posEqual=std::string::npos;
          size_t posComment=std::string::npos;

          bool offsetDone=false;
          bool scaleDone=false;
          bool classValueMapDone=false;
          bool nameVectorDone=false;
          bool bandNamesDone=false;
          while(getline(m_ifstream,csvRecord)){//read a line
            if(!offsetDone){
              //reading offset
              if(csvRecord.find("offset")==std::string::npos)
                continue;
              if(verbose>1)
                std::cout << csvRecord << std::endl;
              lineStream.clear();
              lineStream.str(csvRecord);
              isEqual=false;
              csvstream.clear();
              csvstream.str(csvRecord);
              ncol=0;
              while(getline(csvstream,item,m_fs)){//read a column
                if(verbose)
                  std::cout << item << " ";
                pos=item.find(m_comment);
              ...
              while(getline(csvstream,item,m_fs)){//read a column
                if(verbose)
                  std::cout << item << " ";
                posEqual=item.find(m_equal);
                posComment=item.find(m_comment);
                if(!isEqual){
                  if(posEqual!=std::string::npos){
                    isEqual=true;
                    if(posEqual>0)
                      item=item.substr(posEqual+1);
                    else
                      break;
                    if(verbose)
                      std::cout << "equal found, string is " << item << std::endl;
                  }
                  else
                    continue;//until we found equal character
                }
                if(posComment!=std::string::npos){
                  isComment=true;
                  if(posComment>0)
                    item=item.substr(0,posComment-1);
                  else
                    break;
                  if(verbose)
                    std::cout << "comment found, string is " << item << std::endl;
                }
                //rstrip comma from item
                pos=item.find(',');
                if(pos!=std::string::npos)
                  item=item.substr(0,pos-1);
                float value=string2type<float>(item);

                offset.push_back(value);
              }
              if(offset.size()!=ncol){
                std::string errorString="Error: reading offset";
                throw(errorString);
              }
              if(verbose){
            ...
            if(!scaleDone){
              ...
              //if(csvRecord.find("scale")==std::string::npos)
              //  continue;
              if(verbose>1)
                std::cout << csvRecord << std::endl;
              lineStream.clear();
              ...
              lineStream.str(csvRecord);
              isComment=false;
              while(getline(lineStream,item,m_fs)){//read a column
                if(verbose)
                  std::cout << item << " ";
                posEqual=item.find(m_equal);
                posComment=item.find(m_comment);
              ...
              if(scale.size()!=ncol){
                std::string errorString="Error: reading scale";
                throw(errorString);
              }
              if(verbose){
                ...
                for(int index=0;index<scale.size();++index){
                  if(index<scale.size()-1)
                    std::cout << scale[index] << ", ";
                  else
                    std::cout << scale[index] << std::endl;
                }
              }
              scaleDone=true;
              continue;
            }
            if(!classValueMapDone){
              //reading classValueMap
              //if(csvRecord.find("classValueMap")==std::string::npos)
              //  continue;
              if(verbose>1)
                std::cout << csvRecord << std::endl;
              lineStream.clear();
              ...
              while(getline(lineStream,item,m_fs)){//read a column
                ...
                if(pos!=std::string::npos)
                  item=item.substr(0,pos-1);
                //split item in item1 before ':' and item2 after ':'
                pos=item.find(':');
                std::string key;
                if(pos!=std::string::npos){
                  key=item.substr(0,pos);
                  float value=string2type<float>(item.substr(pos+1));
                  classValueMap[key]=value;
                }
                else{
                  std::string errorString="Error: reading classValueMap (no : found)";
                  throw(errorString);
                }
              }
              if(verbose){
                std::cout << "classValueMap=";
                std::map<std::string,short>::const_iterator mapit=classValueMap.begin();
                while(mapit!=classValueMap.end()){
                  if(mapit!=--classValueMap.end())
              ...
              if(classValueMap.size()!=ncol){
                std::ostringstream errorStream;
                errorStream << "Error: reading classValueMap (ncol: " << classValueMap.size() << ")";
                throw(errorStream.str());
              }
              classValueMapDone=true;
              continue;
            }
            if(!nameVectorDone){
              bool nameVectorFound=false;
              //reading nameVector
              if(csvRecord.find("nameVector")!=std::string::npos)
                nameVectorFound=true;
              if(verbose>1)
                std::cout << csvRecord << std::endl;
              lineStream.clear();
              lineStream.str(csvRecord);
              ...
              while(getline(lineStream,item,m_fs)){//read a column
                ...
                if(pos!=std::string::npos)
                  item=item.substr(0,pos-1);
                nameVector.push_back(item);
              }
              if(nameVector.size()!=ncol){
                std::string errorString="Error: reading nameVector";
                throw(errorString);
              }
              if(verbose){
                std::cout <<  "nameVector:" << std::endl;
                for(int index=0;index<nameVector.size();++index){
                  if(index<nameVector.size()-1)
                    std::cout << nameVector[index] << ", ";
                  else
            ...
            if(!bandNamesDone){
              bool bandNamesFound=false;
              //reading bandNames
              if(csvRecord.find("bandNames")!=std::string::npos)
                bandNamesFound=true;
              if(verbose>1)
                std::cout << csvRecord << std::endl;
              lineStream.clear();
              ...
              while(getline(lineStream,item,m_fs)){//read a column
                ...
                if(pos!=std::string::npos)
                  item=item.substr(0,pos-1);
                bandNames.push_back(item);
              }
              if(bandNames.size()!=ncol){
                std::string errorString="Error: reading bandNames";
                throw(errorString);
              }
              if(verbose){
                std::cout <<  "bandNames:" << std::endl;
                for(int index=0;index<bandNames.size();++index){
                  if(index<bandNames.size()-1)
                    std::cout << bandNames[index] << ", ";
                  else
        ...
        int fann_save_internal_fd(std::ostream &outputStream, struct fann *ann, unsigned int save_as_fixed)
        {
          ...
          if(save_as_fixed)
            {
              ...
              outputStream << FANN_FIX_VERSION << std::endl;
            }
          else
            {
              /* save the version information */
              /* fprintf(conf, FANN_FLO_VERSION "\n"); */
              outputStream << FANN_FLO_VERSION << std::endl;
            }
#else
          /* save the version information */
          /* fprintf(conf, FANN_FIX_VERSION "\n"); */
          ...
          if(save_as_fixed)
            {
              ...
              outputStream << "decimal_point=" << decimal_point << std::endl;
            }
#else
          /* save the decimal_point on a seperate line */
          outputStream << "decimal_point=" << ann->decimal_point << std::endl;

#endif

          /* Save network parameters */
          outputStream << "num_layers=" << (int)(ann->last_layer - ann->first_layer) << std::endl;
          outputStream << "learning_rate=" << ann->learning_rate << std::endl;
          outputStream << "connection_rate=" << ann->connection_rate << std::endl;
          outputStream << "network_type=" << ann->network_type << std::endl;

          outputStream << "learning_momentum=" << ann->learning_momentum << std::endl;
          outputStream << "training_algorithm=" << ann->training_algorithm << std::endl;
          outputStream << "train_error_function=" << ann->train_error_function << std::endl;
          outputStream << "train_stop_function=" << ann->train_stop_function << std::endl;
          outputStream << "cascade_output_change_fraction=" << ann->cascade_output_change_fraction << std::endl;
          outputStream << "quickprop_decay=" << ann->quickprop_decay << std::endl;
          outputStream << "quickprop_mu=" << ann->quickprop_mu << std::endl;
          outputStream << "rprop_increase_factor=" << ann->rprop_increase_factor << std::endl;
          outputStream << "rprop_decrease_factor=" << ann->rprop_decrease_factor << std::endl;
          outputStream << "rprop_delta_min=" << ann->rprop_delta_min << std::endl;
          outputStream << "rprop_delta_max=" << ann->rprop_delta_max << std::endl;
          outputStream << "rprop_delta_zero=" << ann->rprop_delta_zero << std::endl;
          outputStream << "cascade_output_stagnation_epochs=" << ann->cascade_output_stagnation_epochs << std::endl;
...
/* #endif */
          ...
          if(save_as_fixed)
            {
              outputStream << "bit_fail_limit=" << (int) floor((ann->bit_fail_limit * fixed_multiplier) + 0.5) << std::endl;
              outputStream << "cascade_candidate_limit=" << (int) floor((ann->cascade_candidate_limit * fixed_multiplier) + 0.5) << std::endl;
              outputStream << "cascade_weight_multiplier=" << (int) floor((ann->cascade_weight_multiplier * fixed_multiplier) + 0.5) << std::endl;
            }
          else
#endif
            {
              outputStream << "bit_fail_limit=" << ann->bit_fail_limit << std::endl;
              outputStream << "cascade_candidate_limit=" << ann->cascade_candidate_limit << std::endl;
              outputStream << "cascade_weight_multiplier=" << ann->cascade_weight_multiplier * fixed_multiplier << std::endl;
            }

          outputStream << "cascade_activation_functions_count=" << ann->cascade_activation_functions_count << std::endl;
          outputStream << "cascade_activation_functions=";
          for(i = 0; i < ann->cascade_activation_functions_count; i++)
            outputStream << ann->cascade_activation_functions[i] << " ";
          outputStream << std::endl;

          outputStream << "cascade_activation_steepnesses_count=" << ann->cascade_activation_steepnesses_count << std::endl;
          outputStream << "cascade_activation_steepnesses=";
          for(i = 0; i < ann->cascade_activation_steepnesses_count; i++)
            {
#ifndef FIXEDFANN
              if(save_as_fixed)
                outputStream << (int) floor((ann->cascade_activation_steepnesses[i] * fixed_multiplier) + 0.5) << " ";
              else
#endif
                //for floatfann  #define FANNPRINTF "%.20e"
                //for fixedfann  #define FANNPRINTF "%d"
                //for doublefann #define FANNPRINTF "%.20e"
                //fprintf(conf, FANNPRINTF" ", ann->cascade_activation_steepnesses[i]);
                outputStream << ann->cascade_activation_steepnesses[i] <<  " ";
            }
          outputStream << std::endl;

          outputStream << "layer_sizes=";
          for(layer_it = ann->first_layer; layer_it != ann->last_layer; layer_it++)
            {
              ...
              outputStream << (int)(layer_it->last_neuron - layer_it->first_neuron) << " ";
            }
          outputStream << std::endl;
...
#define SCALE_SAVE( what, where )                             \
          outputStream << #what << "_" << #where << "=";      \
          for( i = 0; i < ann->num_##where##put; i++ )        \
            outputStream << ann->what##_##where[ i ] << " ";  \
          outputStream << std::endl;

          if(!save_as_fixed){
            if(ann->scale_mean_in != NULL){
              outputStream << "scale_included=1" << std::endl;
              ...
              outputStream << "scale_included=0" << std::endl;
          }
#undef SCALE_SAVE
#endif

          /* 2.0 */
          outputStream << "neurons (num_inputs, activation_function, activation_steepness)=";
          for(layer_it = ann->first_layer; layer_it != ann->last_layer; layer_it++)
            {
              /* the neurons */
              for(neuron_it = layer_it->first_neuron; neuron_it != layer_it->last_neuron; neuron_it++)
                {
                  ...
                  if(save_as_fixed)
                    {
                      outputStream << "(" << neuron_it->last_con - neuron_it->first_con << ", " << neuron_it->activation_function << ", " << (int) floor((neuron_it->activation_steepness * fixed_multiplier) + 0.5) << ") ";
                    }
                  else
                    {
                      outputStream << "(" << neuron_it->last_con - neuron_it->first_con << ", " << neuron_it->activation_function << ", " << neuron_it->activation_steepness << ") ";
                    }
#else
                  outputStream << "(" << neuron_it->last_con - neuron_it->first_con << ", " << neuron_it->activation_function << ", " << neuron_it->activation_steepness << ") ";
#endif
           ...
           */
          outputStream << "connections (connected_to_neuron, weight)=";
              ...
              if(save_as_fixed)
                {
                  ...
                  outputStream << "(" << (int)(connected_neurons[i] - first_neuron) << ", " << (int) floor((weights[i] * fixed_multiplier) + 0.5) << ") ";
                }
              else
                {
                  /* save the connection "(source weight) " */
                  outputStream << "(" << (int)(connected_neurons[i] - first_neuron) << ", " << weights[i] << ") ";
                }
#else
              /* save the connection "(source weight) " */
              outputStream << "(" << (int)(connected_neurons[i] - first_neuron) << ", " << weights[i] << ") ";
        ...
        bool save(const std::string &configuration_file, const std::vector<double> &offset, const std::vector<double> &scale, std::map<std::string,short> &classValueMap, const std::vector<std::string> &nameVector, const std::vector<std::string> &bandNames){
          bool result=false;
          std::ofstream fileStream(configuration_file);
          if(fileStream)
            result=ann_serialize_model(fileStream, offset, scale, classValueMap, nameVector, bandNames);
          fileStream.close();
          return result;
        }

        bool ann_serialize_model(std::ostream &outputStream, const std::vector<double> &offset, const std::vector<double> &scale, std::map<std::string,short> &classValueMap, const std::vector<std::string> &nameVector, const std::vector<std::string> &bandNames)
        {
          short verbose=0;
          if (ann == NULL)
            {
          ...
          /* FILE *conf = fopen(configuration_file.c_str(), "w+"); */

          /* if(!conf) */
          /*   { */
          /*     fann_error((struct fann_error *) ann, FANN_E_CANT_OPEN_CONFIG_W, configuration_file.c_str()); */
          /*     return false; */
          /*   } */
          /* if(fann_save_internal_fd(ann, conf, configuration_file.c_str(), 0)==-1) */
          if(fann_save_internal_fd(outputStream, ann, 0)==-1)
            return(false);
          if(verbose)
            std::cout << "writing offset: " << offset[0] << " ..." << std::endl;
          outputStream << "offset=";
          for(int index=0;index<offset.size();++index){
            if(index<offset.size()-1)
              outputStream << offset[index] << ", ";
            else
              outputStream << offset[index] << std::endl;
          }
          if(verbose)
            std::cout << "writing scale: " << scale[0] << " ..." << std::endl;
          outputStream << "scale=";
          for(int index=0;index<scale.size();++index){
            if(index<scale.size()-1)
              outputStream << scale[index] << ", ";
            else
              outputStream << scale[index] << std::endl;
          }
          //classValueMap
          if(verbose)
            std::cout << "writing classValueMap: " << classValueMap.begin()->first <<  " ..." << std::endl;
          outputStream << "classValueMap=";
          std::map<std::string,short>::const_iterator mapit=classValueMap.begin();
          while(mapit!=classValueMap.end()){
            if(mapit!=--classValueMap.end())
              outputStream << mapit->first << ":" << mapit->second << ", ";
            else
              outputStream << (mapit->first) << ":" << mapit->second << std::endl;
            ++mapit;
          }
          //nameVector
          if(verbose)
            std::cout << "writing nameVector: " << nameVector[0] <<  " ..." << std::endl;
          outputStream << "nameVector=";
          for(int index=0;index<nameVector.size();++index){
            if(index<nameVector.size()-1)
              outputStream << nameVector[index] << ", ";
            else
              outputStream << nameVector[index] << std::endl;
          }
          //bandNames
          if(bandNames.size()){
            if(verbose)
              std::cout << "writing bandNames: " << bandNames[0] <<  " ..." << std::endl;
            outputStream << "bandNames=";
            for(int index=0;index<bandNames.size();++index){
              if(index<bandNames.size()-1)
                outputStream << bandNames[index] << ", ";
              else
                outputStream << bandNames[index] << std::endl;
            }
          }
          return true;
        }
