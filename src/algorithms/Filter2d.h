...
namespace filter2d
{
  enum FILTER_TYPE { median=200, var=201 , min=202, max=203, sum=204, mean=205, minmax=206, dilate=207, erode=208, close=209, open=210, homog=211, sobelx=212, sobely=213, sobelxy=214, sobelyx=215, smooth=216, density=217, mode=218, mixed=219, threshold=220, ismin=221, ismax=222, heterog=223, order=224, stdev=225, mrf=226, dwt=227, dwti=228, dwt_cut=229, scramble=230, shift=231, linearfeature=232, smoothnodata=233, countid=234, dwt_cut_from=235, savgolay=236, percentile=237, proportion=238, nvalid=239, sauvola=240};
...
public:
  ...
  void dwtCut(Jim& input, Jim& output, const std::string& wavelet_type, int family, double cut, bool verbose=false);
  ...
  void majorVoting(Jim& input, Jim& output, int dim=0,const std::vector<int> &prior=std::vector<int>());
  ...
  void doit(Jim& input, Jim& output, const std::string& method, int dim, short down=1, bool disc=false);
  void doit(Jim& input, Jim& output, const std::string& method, int dimX, int dimY, short down=1, bool disc=false);
  void mrf(Jim& input, Jim& output, int dimX, int dimY, double beta, bool eightConnectivity=true, short down=1, bool verbose=false);
  void mrf(Jim& input, Jim& output, int dimX, int dimY, Vector2d<double> beta, bool eightConnectivity=true, short down=1, bool verbose=false);
  ...
  void median(Jim& input, Jim& output, int dim, bool disc=false);
  void var(Jim& input, Jim& output, int dim, bool disc=false);
  void morphology(Jim& input, Jim& output, const std::string& method, int dimX, int dimY, const std::vector<double> &angle=std::vector<double>(), bool disc=false);
  ...
  template<class T> void dsm2shadow(const Vector2d<T>& input, Vector2d<T>& output, double sza, double saa, double pixelSize, short shadowFlag=1);
  void dsm2shadow(Jim& input, Jim& output, double sza, double saa, double pixelSize, short shadowFlag=1);
  //  void dwt_texture(Jim& input, Jim& output, int dim, int scale, int down=1, int iband=0, bool verbose=false);
  void shift(Jim& input, Jim& output, double offsetX=0, double offsetY=0, double randomSigma=0, RESAMPLE resample=BILINEAR, bool verbose=false);
      ...
      if(currentMasked){
        ...
        if(statBuffer.size()){
          ...
          case(filter2d::erode):
            ...
            /* if(m_class.size()){ */
            /*   if(stat.mymin(statBuffer)+hThreshold>0) */
            /*     output[y][x]=currentValue; */
            /*   else{ */
            /*     output[y][x]=m_noDataValues[0]; */
            /*     ++nchange; */
            /*   } */
            /* } */
            /* else if(output[y][x]>stat.mymin(statBuffer)+hThreshold){ */
            /*   output[y][x]=(m_class.size()) ? currentValue : stat.mymin(statBuffer); */
            /*   ++nchange; */
            /* } */
            break;
          default:
 ...
 template<class T> unsigned long int Filter2d::dsm2dtm_swne(const Vector2d<T>& inputDSM, Vector2d<T>& outputMask, double hThreshold, int nlimit, int dim)
{
  ...
  template<class T> void Filter2d::dsm2shadow(const Vector2d<T>& input, Vector2d<T>& output, double sza, double saa, double pixelSize, short shadowFlag)
{
